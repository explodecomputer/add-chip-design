...
One of the goals of the ADD cohort is to include a larger proportion of non-European ancestry individuals than previous studies have managed. We need to ensure that the array design is able to obtain reasonable imputation accuracy of non-Europeans, if not parity with Europeans. This must be done without abrogating the imputation quality of the European samples, who will constitute the majority of the data.

There are two broad approaches:

1. Variant selection for post-imputation checking of reliable performance. This is what was originally discussed on the call
2. Variant selection optimised for imputation accuracy

## Variant selection for post-imputation checking of reliable performance

**Suggestion 1:** 

Include a set of ancestry informative markers on the SNP chip and then exclude them when performing imputation, in order to evaluate how well they are imputed against the known genotype value. These tend to be variants that have high Fst in global population samples. We could ask whether variants that differentiate a pair of populations have been as well imputed as variants that differentiate other pairs of populations. 
...
Maf - Info relationship for each sub population. No need for specific variants to be included
...
HRC - http://www.haplotype-reference-consortium.org/participating-cohorts
